...
const EmptyState = ( { title, search, buttonLink, buttonText}: EmptyStateProps) => {
  return (
    <section className="flex-center flex-col size-full gap-3">
       ...
       <div className='flex-center w-full max-w-[264px] flex-col gap-3'>
        <h1 className="text-16 text-center font-medium text-white-1">{title}</h1>
        {search && (
            <p className='text-16 text-center font-medium text-white-2'>Try adjusting your search parameters to find what you are looking for.</p>
        )}
        {buttonLink && (
